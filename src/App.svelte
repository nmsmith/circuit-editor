<script lang="ts">
   ...
   // ✨ A magical easing function for aesthetically-pleasing snapping. ✨
   // (Converts mouse distance to visual distance.)
   type EasingOutcome =
      | { outcome: "snapped"; point: Point }
      | { outcome: "eased"; point: Point }
      | { outcome: "no easing"; point: Point }
   function computeEasing(source: Point, target: Point): EasingOutcome {
      // Easing constants
      const a =
         (snapRadius - snapGap) /
   ...
   function closestEndpoint(
      point: Point
   ): { endpoint: Point; sqDistance: number } | null {
      let closest: { endpoint: Point; sqDistance: number } | null = null
      for (let p of circuit.keys()) {
         let sqDistance = p.sqDistanceFrom(point)
         if (!closest || sqDistance < closest.sqDistance) {
            closest = { endpoint: p, sqDistance }
         }
      }
      return closest
   }
   function snapToClosestEndpoint(point: Point): Point | null {
      let closest = closestEndpoint(point)
      if (closest && closest.sqDistance < sqSnapRadius) {
         return closest.endpoint
      } else return null
   }
   function easeToClosestEndpoint(point: Point): EasingOutcome {
      let closest = closestEndpoint(point)
      return closest
         ? computeEasing(point, closest.endpoint)
         : { outcome: "no easing", point }
   }
   function snapToClosestRuler(point: Point): Point | null {
      // TODO: Implement this.
      return null
   }
   function easeToClosestRuler(
      point: Point,
      generatingPoint?: Point // Only snap to rulers generated by this point
   ): Point {
      let closest:
         | { ruler: Ruler; sqDistance: number; projection: Point }
         | undefined
      for (let ruler of activeRulers) {
         ...
         let sqDistance = rejection.sqLength()
         if (
            sqDistance < sqEaseRadius &&
            (!closest || sqDistance < closest.sqDistance)
         ) {
            closest = {
               ruler,
               sqDistance,
               projection: point.displaceBy(rejection),
            }
         }
      }
      if (closest) return computeEasing(point, closest.projection).point
      else return point
   }
   // Snap the point to a nearby line segment, if possible.
   // Otherwise, return the same point.
   function snapToClosestSegment(
      point: Point,
      stretchAxis?: Axis
      ...
      for (let segment of segments) {
         ...
         if (sqDistance > sqSnapRadius) continue
      ...
      if (closest) {
         return [closest.point, closest.segment]
      } else return [point, undefined]
   }
   // --------------- State ---------------
   ...
   $: {
      ...
      if (draw) {
         ...
         let p = snapToClosestEndpoint(mouse)
         if (p === null) {
            let s = snapToClosestSegment(mouse)[1]
            if (s) {
               highlighted.add(s)
   ...
   function beginDraw(clickPosition: Point) {
      let start = snapToClosestEndpoint(clickPosition)
      // If we couldn't snap to an endpoint, try snapping to a segment.
      if (start === null) {
         let [start, startSegment] = snapToClosestSegment(clickPosition)
         draw = {
            start,
            startIsNew: true,
            startSegment,
   ...
   $: /* On a change to 'draw' or 'mouse' */ {
      if (draw) {
         ...
         let ease = easeToClosestEndpoint(mouse)
         draw.end = ease.point
         // Prioritize endpoint snapping over everything else.
         if (ease.outcome === "snapped") {
            draw.endIsNew = false
            ...
            draw.end =
               mouse.sqDistanceFrom(draw.start) < sqEaseRadius
                  ? computeEasing(mouse, draw.start).point
                  : mouse
            draw.end = easeToClosestRuler(draw.end)
            draw.axis = tryRoundingToExistingAxis(
               Axis.fromVector(draw.end.displacementFrom(draw.start))
            )
            ;[draw.end, draw.endSegment] = snapToClosestSegment(
               draw.end,
               draw.axis
            )
         }
   ...
   function beginMove(clickPosition: Point) {
      let start = snapToClosestEndpoint(clickPosition)
      if (start === null) {
         let [start, segment] = snapToClosestSegment(clickPosition)
         if (segment) {
            move = {
      ...
      if (move) {
         ...
         move.end =
            mouse.sqDistanceFrom(move.start) < sqEaseRadius
               ? computeEasing(mouse, move.start).point
               : mouse
         ...
         move.end = easeToClosestRuler(m, move.points[0]).displaceBy(
            d.scaleBy(-1)
         )
